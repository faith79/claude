...
- [x] 일기 수정
- [x] 일기 삭제
- [x] 달력(Monthly Calendar) 형태 일기 목록 조회
- [x] 일기 작성 날짜에 감정 이모티콘 표시 (달력 셀)
...
| FR-06 | Monthly Calendar UI — 한 화면에 한 달만 표시하는 달력 형태 일기 목록 | High | Pending |
| FR-06a | 앱 진입 시 현재 달(오늘 날짜 기준)을 기본으로 표시 | High | Pending |
| FR-06b | 상단 헤더에 "◀ 2026년 5월 ▶" 형태로 이전/다음 달 이동 버튼 제공 | High | Pending |
| FR-06c | 이전/다음 버튼 탭 시 해당 월로 전환 (한 번에 1개월 이동) | High | Pending |
| FR-06d | 일기 작성 날짜 달력 셀에 감정 이모티콘 표시 (감정 없으면 기본 점 표시) | High | Pending |
| FR-06e | 달력 날짜 탭 → 해당 날짜 일기 상세/작성 화면 이동 | High | Pending |
...
- [ ] FR-01~FR-12 + FR-06a~06e 모두 구현 및 에뮬레이터 동작 확인
- [ ] Firebase Auth 로그인·로그아웃 정상 동작
- [ ] Firestore CRUD 정상 동작 (본인 데이터만 접근)
- [ ] 이미지 업로드/표시 정상 동작
- [ ] 감정 태그 저장/표시 정상 동작
- [ ] 앱 진입 시 현재 달 달력 자동 표시
- [ ] 이전/다음 달 버튼으로 1개월씩 이동
- [ ] 한 화면에 한 달 달력만 표시 (스크롤 없이 한 페이지)
- [ ] 일기 작성 날짜 달력 셀에 감정 이모티콘 올바르게 표시
- [ ] 검색 필터 동작
- [ ] 일일 알림 수신 확인
...
├── ui/
│   ├── auth/          ← 로그인/회원가입 화면
│   ├── home/          ← 달력(Monthly Calendar) 일기 목록 화면
│   ├── editor/        ← 일기 작성/수정 화면
│   └── detail/        ← 일기 상세 화면
├── viewmodel/
│   ├── AuthViewModel
